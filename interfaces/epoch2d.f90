...
INTERFACE
  ...
  SUBROUTINE calculate_2d(rank, nproc, laser_start, laser_end, &
    fwhm_time, t_0, omega, amplitude, horizontal_pos, w_0, direction, id, phase, boundary, focus, horizontal_min, horizontal_max, &
  ...
  SUBROUTINE Maxwell_consistent_computation_of_EM_fields
    ...
    DO WHILE(ASSOCIATED(current))
      CALL calculate_2d(rank, nproc, current%t_start, current%t_end, &
        ...
        current%id, current%phi, x_min, current%focus, y_min, y_max, ny_global, cpml_thickness, t_end, &
    ...
    DO WHILE(ASSOCIATED(current))
      CALL calculate_2d(rank, nproc, current%t_start, current%t_end, &
        current%fwhm_time, current%t_0, current%omega, current%amp, current%horizontal_pos, current%w_0, -1, &
        current%id, current%phi, x_max, current%focus, y_min, y_max, ny_global, cpml_thickness, t_end, &
        dy, dt, TRIM(data_dir)//C_NULL_CHAR)
      current => current%next
    ENDDO
    ...
    DO WHILE(ASSOCIATED(current))
      CALL calculate_2d(rank, nproc, current%t_start, current%t_end, &
        current%fwhm_time, current%t_0, current%omega, current%amp, current%horizontal_pos, current%w_0, 1, &
        current%id, current%phi, y_min, current%focus, x_min, x_max, nx_global, cpml_thickness, t_end, &
    ...
    DO WHILE(ASSOCIATED(current))
      CALL calculate_2d(rank, nproc, current%t_start, current%t_end, &
        current%fwhm_time, current%t_0, current%omega, current%amp, current%horizontal_pos, current%w_0, -1, &
        current%id, current%phi, y_max, current%focus, x_min, x_max, nx_global, cpml_thickness, t_end, &
        dx, dt, TRIM(data_dir)//C_NULL_CHAR)
      current => current%next
    ENDDO
