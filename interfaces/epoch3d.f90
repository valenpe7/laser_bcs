...
INTERFACE
  ...
  SUBROUTINE calculate_3d(rank, nproc, laser_start, laser_end, &
      ...
      phase, boundary, focus, horizontal_min, horizontal_max, vertical_min, &
  ...
  SUBROUTINE Maxwell_consistent_computation_of_EM_fields
    ...
    DO WHILE(ASSOCIATED(current))
      CALL calculate_3d(rank, nproc, current%t_start, current%t_end, &
        ...
        current%vertical_pos, current%w_0, 1, current%id, current%phi, x_min, current%focus, y_min, y_max, &
    ...
    DO WHILE(ASSOCIATED(current))
      CALL calculate_3d(rank, nproc, current%t_start, current%t_end, &
        current%fwhm_time, current%t_0, current%omega, current%amp, current%horizontal_pos, &
        current%vertical_pos, current%w_0, -1, current%id, current%phi, x_max, current%focus, y_min, y_max, &
        z_min, z_max, ny_global, nz_global, cpml_thickness, t_end, dy, dz, dt, TRIM(data_dir)//C_NULL_CHAR)
      current => current%next
    ENDDO
    ...
    DO WHILE(ASSOCIATED(current))
      CALL calculate_3d(rank, nproc, current%t_start, current%t_end, &
        current%fwhm_time, current%t_0, current%omega, current%amp, current%horizontal_pos, &
        current%vertical_pos, current%w_0, 1, current%id, current%phi, y_min, current%focus, x_min, x_max, &
    ...
    DO WHILE(ASSOCIATED(current))
      CALL calculate_3d(rank, nproc, current%t_start, current%t_end, &
        current%fwhm_time, current%t_0, current%omega, current%amp, current%horizontal_pos, &
        current%vertical_pos, current%w_0, -1, current%id, current%phi, y_max, current%focus, x_min, x_max, &
        z_min, z_max, nx_global, nz_global, cpml_thickness, t_end, dx, dz, dt, TRIM(data_dir)//C_NULL_CHAR)
      current => current%next
    ENDDO
    ...
    DO WHILE(ASSOCIATED(current))
      CALL calculate_3d(rank, nproc, current%t_start, current%t_end, &
        current%fwhm_time, current%t_0, current%omega, current%amp, current%horizontal_pos, &
        current%vertical_pos, current%w_0, 1, current%id, current%phi, z_min, current%focus, x_min, x_max, &
    ...
    DO WHILE(ASSOCIATED(current))
      CALL calculate_3d(rank, nproc, current%t_start, current%t_end, &
        current%fwhm_time, current%t_0, current%omega, current%amp, current%horizontal_pos, &
        current%vertical_pos, current%w_0, -1, current%id, current%phi, z_max, current%focus, x_min, x_max, &
        y_min, y_max, nx_global, ny_global, cpml_thickness, t_end, dx, dy, dt, TRIM(data_dir)//C_NULL_CHAR)
      current => current%next
    ENDDO
