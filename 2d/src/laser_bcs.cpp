...
void laser_bcs::set_param(const domain_param* dp, const laser_param* lp) {
	this->domain = dp;
	this->laser = lp;
	this->e_x.resize(boost::extents[this->domain->nx][this->domain->nt]);
	this->e_y.resize(boost::extents[this->domain->nx][this->domain->nt]);
	this->e_z.resize(boost::extents[this->domain->nx][this->domain->nt]);
	this->b_x.resize(boost::extents[this->domain->nx][this->domain->nt]);
	this->b_y.resize(boost::extents[this->domain->nx][this->domain->nt]);
	this->b_z.resize(boost::extents[this->domain->nx][this->domain->nt]);
...
#pragma omp parallel for schedule(static)
	...
	this->k_z.resize(boost::extents[this->domain->nx][this->domain->nt]);
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
	for(auto i = 0; i < this->domain->nx; i++) {
...
void laser_bcs::prescribe_field_at_focus(array_2d<complex>& field) const {
...
#pragma omp parallel for schedule(static)
	...
	for(auto i = 0; i < this->domain->nx; i++) {
		for(auto j = 0; j < this->domain->nt; j++) {
			if((this->domain->t_coord[j] - this->domain->time_shift) >= this->laser->t_start && (this->domain->t_coord[j] - this->domain->time_shift) <= this->laser->t_end) {
				field[i][j] = {this->laser->amplitude * exp(
					- pow((this->domain->x_coord[i] - this->laser->x_0) / this->laser->w_0, 2)
					- pow((this->domain->t_coord[j] - this->laser->t_0 - this->domain->time_shift) * (2.0 * sqrt(log(2.0)))
						/ this->laser->fwhm_time, 2)) * cos(this->laser->omega * this->domain->t_coord[j]), 0.0};
...
void laser_bcs::dft_time(array_2d<complex>& field, int sign) const {
	std::vector<complex> global_extent(this->domain->nt_global);
	fft::create_plan_1d(this->domain->nt_global, sign);
	for(auto i = 0; i < this->domain->nx; i++) {
		MPI_Gatherv(field[boost::indices[i][range()]].origin(), this->domain->nt, MPI_DOUBLE_COMPLEX, global_extent.data(), this->domain->t_counts.data(), this->domain->t_displs.data(), MPI_DOUBLE_COMPLEX, 0, MPI_COMM_WORLD);
		if(this->domain->rank == 0) {
			global_extent = fft::execute_plan(global_extent);
		}
		MPI_Scatterv(global_extent.data(), this->domain->t_counts.data(), this->domain->t_displs.data(), MPI_DOUBLE_COMPLEX, field[boost::indices[i][range()]].origin(), this->domain->nt, MPI_DOUBLE_COMPLEX, 0, MPI_COMM_WORLD);
	}
	fft::destroy_plan();
}

void laser_bcs::dft_space(array_2d<complex>& field, int sign) const {
	fft::create_plan_1d(this->domain->nx, sign);
	for(auto i = 0; i < this->domain->nt; i++) {
		view_1d row = field[boost::indices[range()][i]];
		tools::vec_to_row(row, fft::execute_plan(tools::row_to_vec(row)));
	}
	fft::destroy_plan();
}
...
#pragma omp parallel for schedule(static)
	...
	for(auto i = 0; i < this->domain->nx; i++) {
		for(auto j = 0; j < this->domain->nt; j++) {
			if(this->k_z[i][j] > 0) {
				this->e_x[i][j] *= exp(constants::imag_unit * this->k_z[i][j] * (this->domain->z_boundary - this->domain->z_focus));
				this->e_y[i][j] *= exp(constants::imag_unit * this->k_z[i][j] * (this->domain->z_boundary - this->domain->z_focus));
			} else {
...
void laser_bcs::dump_fields(std::string output_path) const {
	...
	std::array<int, 4> local_extent = {0, this->domain->nx - 1, this->domain->nt_start, this->domain->nt_start + this->domain->nt - 1};
	std::array<int, 4> global_extent = {0, this->domain->nx - 1, 0, this->domain->nt_global - this->domain->ghost_cells - 1};
...
void laser_bcs::dump_to_shared_file(array_2d<complex> field, std::array<int, 4> local_extent, std::array<int, 4> global_extent, std::string filename) const {
	MPI_File file;
	MPI_Offset offset = 0;
	MPI_Status status;
...
void laser_bcs::run_computation() {
	...
	tools::multiply_array(this->e_x, 1.0 / (this->domain->nx * this->domain->nt_global));
	tools::multiply_array(this->e_y, 1.0 / (this->domain->nx * this->domain->nt_global));
	tools::multiply_array(this->e_z, 1.0 / (this->domain->nx * this->domain->nt_global));
	tools::multiply_array(this->b_x, 1.0 / (this->domain->nx * this->domain->nt_global));
	tools::multiply_array(this->b_y, 1.0 / (this->domain->nx * this->domain->nt_global));
	tools::multiply_array(this->b_z, 1.0 / (this->domain->nx * this->domain->nt_global));
