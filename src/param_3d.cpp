...
void param_3d::set_laser(double t_start, double t_end, double fwhm_time, double t_0, double x_0, double y_0, double omega, double amp, double w_0, int order, int direction, int id, double phase) {
	if (t_start > t_end || fwhm_time < 0 || omega < 0 || amp < 0 || w_0 < 2.0 * constants::c / omega || order < 2) {
		std::cerr << "error: bad value" << std::endl;
		return;
	}
